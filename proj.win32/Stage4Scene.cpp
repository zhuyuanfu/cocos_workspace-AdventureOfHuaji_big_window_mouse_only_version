...
Scene* Stage4Scene::createScene()
{
	return Stage4Scene::create();
}
...
bool Stage4Scene::init()
{
	...
	auto closeItem = MenuItemImage::create(
		"CloseNormal.png",
		"CloseSelected.png",
		CC_CALLBACK_1(Stage4Scene::menuCloseCallback, this));
	...
	auto label = Label::createWithTTF("Bullets are step-stones if carefully dealt with!", "fonts/Marker Felt.ttf", 48);
	if (label == nullptr)
	{
	...
	auto stage4Label = Label::createWithTTF("Stage4", "fonts/Marker Felt.ttf", 24);
	stage4Label->setPosition(stage4Label->getContentSize().width / 2, visibleSize.height - stage4Label->getContentSize().height / 2);
	this->addChild(stage4Label);
	...
	//huajiSprite->setPosition(visibleSize.width - huajiSprite->getContentSize().width * 5 / 2, 7.5 * huajiSprite->getContentSize().height);
	...
	for (int i = 6; i < 13; i++){
		Sprite* groundSprite = Sprite::create("ground_double.png");
		groundSprite->setPosition(i * groundSprite->getContentSize().width + 6 * groundSprite->getContentSize().width, 6.5 * groundSprite->getContentSize().height);
		groundSprite->setTag(i);
	...
	meiguiSprite->setPosition(visibleSize.width - meiguiSprite->getContentSize().width * 3 / 2, 7.5 * meiguiSprite->getContentSize().height);
	meiguiSprite->setTag(101);
	this->addChild(meiguiSprite, 1);

	this->bullet_tag_begin = 200;
	this->bullet_tag_end = 207;
	//add a bullet for tests
	BulletSprite * bulletSprite = BulletSprite::create("bullet_head_up_double.png");
	bulletSprite->setPosition(350, visibleSize.height * 35 / 100);
	bulletSprite->setTag(200);
	bulletSprite->setRotation(-90);
	bulletSprite->current_speed_x = -2;
	bulletSprite->speed = 4;
	bulletSprite->launch_position_x = visibleSize.width + 90;
	bulletSprite->launch_position_y = visibleSize.height * 35 / 100;
	this->addChild(bulletSprite);

	//add a bullet for tests
	BulletSprite * bulletSprite2 = BulletSprite::create("bullet_head_up_double.png");
	bulletSprite2->setPosition(201, visibleSize.height * 40 / 100);
	bulletSprite2->setTag(201);
	bulletSprite2->setRotation(-90);
	bulletSprite2->current_speed_x = -3;
	bulletSprite2->speed = 5;
	bulletSprite2->launch_position_x = visibleSize.width + 90;
	bulletSprite2->launch_position_y = visibleSize.height * 40 / 100;
	this->addChild(bulletSprite2);

	//add a bullet for tests
	BulletSprite * bulletSprite3 = BulletSprite::create("bullet_head_up_double.png");
	bulletSprite3->setPosition(400, visibleSize.height * 50 / 100);
	bulletSprite3->setTag(202);
	bulletSprite3->setRotation(-90);
	bulletSprite3->current_speed_x = -4;
	bulletSprite3->speed = 6;
	bulletSprite3->launch_position_x = visibleSize.width + 90;
	bulletSprite3->launch_position_y = visibleSize.height * 50 / 100;
	this->addChild(bulletSprite3);

	//add a bullet for tests
	BulletSprite * bulletSprite4 = BulletSprite::create("bullet_head_up_double.png");
	bulletSprite4->setPosition(500, visibleSize.height * 30 / 100);
	bulletSprite4->setTag(203);
	bulletSprite4->setRotation(-90);
	bulletSprite4->current_speed_x = -2;
	bulletSprite4->speed = 3;
	bulletSprite4->launch_position_x = visibleSize.width + 90;
	bulletSprite4->launch_position_y = visibleSize.height * 30 / 100;
	this->addChild(bulletSprite4);

	//add a bullet for tests
	BulletSprite * bulletSprite5 = BulletSprite::create("bullet_head_up_double.png");
	bulletSprite5->setPosition(550, visibleSize.height * 20 / 100);
	bulletSprite5->setTag(204);
	bulletSprite5->setRotation(-90);
	bulletSprite5->current_speed_x = -4;
	bulletSprite5->speed = 9;
	bulletSprite5->launch_position_x = visibleSize.width + 90;
	bulletSprite5->launch_position_y = visibleSize.height * 20 / 100;
	this->addChild(bulletSprite5);

	//add a bullet for tests
	BulletSprite * bulletSprite6 = BulletSprite::create("bullet_head_up_double.png");
	bulletSprite6->setPosition(150, visibleSize.height * 25 / 100);
	bulletSprite6->setTag(205);
	bulletSprite6->setRotation(-90);
	bulletSprite6->current_speed_x = -2;
	bulletSprite6->speed = 5;
	bulletSprite6->launch_position_x = visibleSize.width + 90;
	bulletSprite6->launch_position_y = visibleSize.height * 25 / 100;
	this->addChild(bulletSprite6);

	//add a bullet for tests
	BulletSprite * bulletSprite7 = BulletSprite::create("bullet_head_up_double.png");
	bulletSprite7->setPosition(550, visibleSize.height * 45 / 100);
	bulletSprite7->setTag(206);
	bulletSprite7->setRotation(-90);
	bulletSprite7->current_speed_x = -3;
	bulletSprite7->speed = 7;
	bulletSprite7->launch_position_x = visibleSize.width + 90;
	bulletSprite7->launch_position_y = visibleSize.height * 45 / 100;
	this->addChild(bulletSprite7);

	//add a bullet for tests
	BulletSprite * bulletSprite8 = BulletSprite::create("bullet_head_up_double.png");
	bulletSprite8->setPosition(850, visibleSize.height * 45 / 100);
	bulletSprite8->setTag(207);
	bulletSprite8->setRotation(-90);
	bulletSprite8->current_speed_x = -3;
	bulletSprite8->speed = 8;
	bulletSprite8->launch_position_x = visibleSize.width + 90;
	bulletSprite8->launch_position_y = visibleSize.height * 45 / 100;
	this->addChild(bulletSprite8);
		...
		if (mouseButton == EventMouse::MouseButton::BUTTON_LEFT){
			...
			if (this->bullet_time){
				...
				*/
				...
				endPointSprite->runAction(fadeOutAction);
				...
				if (deltax == 0){
					...
					double divider = 1 + pow(deltay / deltax, 2);
	...
	this->schedule(schedule_selector(Stage4Scene::update));
	return true;
}


void Stage4Scene::menuCloseCallback(Ref* pSender)
{
...
void Stage4Scene::update(float dt){
	...
	if (std::abs(huaji->getPositionX() - meigui->getPositionX()) < 15 && std::abs(huaji->getPositionY() - meigui->getPositionY()) < 15){
		...
		Director::getInstance()->replaceScene(Stage5Scene::create());
	}
...
void Stage4Scene::updateIfHuajiIsfOnTheGroundAndResetHuajisYPosition(){
...
void Stage4Scene::updateBulletPositionAndResetThemIfTheyFlyOutOfTheScreen(){
	auto visibleSize = Director::getInstance()->getVisibleSize();
	int bulletTagBegin = this->bullet_tag_begin;
	int bulletTagEnd = this->bullet_tag_end;
	for (int i = bulletTagBegin; i < bulletTagEnd + 1; i++){
